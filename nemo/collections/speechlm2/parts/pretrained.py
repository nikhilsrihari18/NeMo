...
def load_pretrained_hf(model_path_or_name: str, pretrained_weights: bool = True, dtype=torch.float32):
    ...
    if pretrained_weights:
        return AutoModelForCausalLM.from_pretrained(pretrained_model_name_or_path=model_path_or_name, torch_dtype=dtype, trust_remote_code=False)
    else:
        config = AutoConfig.from_pretrained(pretrained_model_name_or_path=model_path_or_name)
