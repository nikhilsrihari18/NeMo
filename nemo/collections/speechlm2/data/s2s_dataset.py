...
def build_aligned_tokens(
    ...
    if superv_start_pos + start_pos >= tokens_len + 3:  # +3 to avoid verbose warnings
        logging.warning(
    ...
    for supervision in cut.supervisions:
        if roles is None or supervision.speaker in roles:
            ...
            if use_alignment_items and supervision.alignment is not None and len(supervision.alignment['word']) > 0:                
                ...
                # Add bos tokens
                fst_pos -= len(bos_ids) 
                tokens[fst_pos:fst_pos+len(bos_ids)] = torch.as_tensor(bos_ids, dtype=tokens.dtype, device=tokens.device)
