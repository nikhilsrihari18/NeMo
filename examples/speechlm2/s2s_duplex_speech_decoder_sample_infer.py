...
class DuplexS2SSpeechDecoderSampleInference:
    ...
    def __init__(self, model_config_path: str, model_ckpt_path: str, ignore_speech_gen: bool):
        logging.info("Initializing DuplexS2SSpeechDecoderSampleInference...")
        self.model_config_path = model_config_path
        self.model_ckpt_path = model_ckpt_path
        self.ignore_speech_gen = ignore_speech_gen
        ...
        if "ignore_speech_gen" in self.model_config:
            if self.model_config["ignore_speech_gen"]:
                if not self.ignore_speech_gen:
                    logging.info("ignore_speech_gen is True in the model config indicating audio will not be generated by this model. ignore_speech_gen can not be False in the command line")
                    self.ignore_speech_gen = True
            else:
                self.model_config["ignore_speech_gen"] = self.ignore_speech_gen
        else:
            self.ignore_speech_gen = False
            logging.info("ignore_speech_gen is not in the model config. Setting ignore_speech_gen to False")
    ...
    def save_output_audio(self, pred_audio_path: str, pred_audio_data: torch.tensor):
        """
            Unused at this time. Also untested at this time.
        """
        if len(pred_audio_data.shape) > 1:
            pred_audio_data = pred_audio_data[0]  # Take first batch
    ...
    def save_output_text(self, text_path: str, text: str):
    ...
    def run_inference(self, audio_path: str = None, input_text: str = None):
        ...
        if input_text:
            ...
            return response, input_text, None
        else:
            ...
            return response, None, input_signal
...
def main():
    ...
    parser.add_argument("--ignore_speech_gen", action="store_true", help="Boolean to decide if audio should be generated or not")
    parser.add_argument("--output_text_path", type=str, required=False, default=None, help="Path to the output text txt file")
    parser.add_argument("--output_audio_path", type=str, required=False, default=None, help="Path to the combined input and output audio wav file")
    ...
    if not args.ignore_speech_gen:
        assert args.output_audio_path is not None, "output_audio_path must be provided if ignore_speech_gen is False"
    else:
        assert args.output_audio_path is None, "output_audio_path must be None if ignore_speech_gen is True"

    inference = DuplexS2SSpeechDecoderSampleInference(args.model_config_path, args.model_ckpt_path, args.ignore_speech_gen)

    response, _, input_signal = inference.run_inference(args.input_audio_path, args.input_text)

    logging.info(f"Response Text: {response['text']}")
    if args.output_text_path:
        inference.save_output_text(args.output_text_path, response['text'])

    if not inference.ignore_speech_gen:
        if response['audio'] is not None:
            inference.save_combined_audio(args.output_audio_path, response['audio'], input_signal)
        else:
            logging.info("No audio was generated by the model")
